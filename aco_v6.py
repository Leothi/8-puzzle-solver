...
class ACOSolver(NPuzzleBase):
    ...
    def solve(cls, self, matrix):
        ...
        while n != 0:
            ...
            for diff_num in diff_nums:
                if diff_num == 0:
                    ...
                    for move in self.actions:
                        temp_data = self.move_tile(
                            move, current_matrix, diff_num)
                        if temp_data is not None:
            ...
            # matriz de fero descrescente
            list_biggest = self.sorted_phero_matrix()
            logger.debug(f"LISTA = {list_biggest}")

            # maior fero
            mov_sequence.append(list_biggest[0])
            # logger.debug(f"SEQUENCIA = {mov_sequence}")
            
            # movimentando a matrix atual
            ...
            # zerando a matriz para proximo passo
            # self.phero_matrix = self.create_phero_matrix()
            logger.debug(f"\n{self.phero_matrix}")
        ...
        # for seq in mov_sequence:
        #     number = self.get_value_from_index(current_matrix, seq[0])
        #     logger.debug(f"SEQUENCIA = {seq}, NUMBER= {number}")
        #     new_matrix = self.move_tile(move, current_matrix, number)
        #     if temp_matrix is not None:
        #         diff_index, n = self.get_diff_from_target(
        #             new_matrix)
        #         if n != 0:
